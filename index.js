...
server.on('connect', (req, clientSocket, head) => {
    ...
    if (req.headers['proxy-authorization']) {
        const [, credentionalsBase64] = req.headers['proxy-authorization'].split(' ');
        const credentionals = Buffer.from(credentionalsBase64, 'base64').toString('utf-8');
        const [userName, password] = credentionals.split(':');
